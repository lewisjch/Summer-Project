...
particles forming the cluster and outputs the volume and density of
the cluster and the density of the exterior space.
...
def main():
    ...
    for p in range(len(prpr)):
        for N in range(len(Nprot)):
            ...
            outputfilename = 'density_prpr'+str(prpr[p])+'_Nprot'+str(Nprot[N])
            ...
            for f in range(Nframes):
                ...
                new_density_outside = (Nprot[N]-len(new_cluster_ids)) / (box_size[0] * box_size[1] * box_size[2] - new_droplet_volume)
    
        
                #add values to list to find averages late
                cluster_radii.append(new_droplet_radius)
                cluster_densities.append(new_droplet_density)
                exterior_cluster_densities.append(new_density_outside)
    
        
            # initialise variables 
            avg_droplet_radius = 0.0
            avg_droplet_density = 0.0
            avg_exterior_density = 0.0
        
            for i in range(Nframes):
                avg_droplet_radius += cluster_radii[i]/Nframes
                avg_droplet_density += cluster_densities[i]/Nframes
                avg_exterior_density += exterior_cluster_densities[i]/Nframes
    
    
            ouf.write(f"{'prpr'}, {'Nprot'}, {'Radius'}, {'Density of Droplet'}, {'Density Outside Droplet'}\n")   
            ouf.write(f"{prpr[p]}, {Nprot[N]}, {avg_droplet_radius}, {avg_droplet_density}, {avg_exterior_density}\n")
